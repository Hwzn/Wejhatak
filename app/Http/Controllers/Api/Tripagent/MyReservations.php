...
class MyReservations extends Controller
{
    ...
    public function package_reservations($lang,$page)
    {
      $HostName=request()->getHttpHost();
      $Tripagent_id=Auth::user()->id;
      $lang=strtolower($lang);
    ...
    public function change_reservationstatus($id,$status)
    {
      ...
      {
        $data['user_reservations']=Booking::select('status','User_id ')->where('Tripagent_id',$Tripagent_id)
                                           ->where('id',$id)
                                           ->first();
        try{
         DB::beginTransaction();
        ...
        if($status=='accepted') $status_ar='تم إستلامه';elseif($status=='completed')$status_ar='تم التأكيد';
                               elseif($status=='refused')$status_ar='تم رفض الطلب';else $status_ar='تحت المراجعة';
          //notficaions
          if($user_id !==Null)
          {
             ...
             $message_content=[
                'en'=>"Status Changed to $status For Your Reservation number  $Booking_number ",
               //  'ar'=>'تم ارسال طلب ' . $service_name . 'من ' . $user_name . 'برقم # ' . $booking->id
                'ar'=>"تم تغيير حالة طلب الحجز رقم $Booking_number"." إلي ".$status_ar,
               ];
            $this->sendnotification($user_id,null,$noification_title,$notification_body,$user_tokens,$message_content);
